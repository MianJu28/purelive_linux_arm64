...
class _WebCookieCapturePageState extends State<WebCookieCapturePage> {
  ...
  /// 维护分支定制：Linux 构建剔除了 flutter_inappwebview 的平台实现
  /// （WPE WebKit 2.40 依赖不可用）。在这些平台上实例化 InAppWebView
  /// 会抛 MissingPluginException，因此降级为提示页，用户改走原有的
  /// 手动粘贴 Cookie 入口；其余平台行为与上游一致。
  late final bool _webViewAvailable =
      PlatformUtils.isAndroid || PlatformUtils.isWindows || PlatformUtils.isMacOS || PlatformUtils.isIOS;
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: !_webViewAvailable
                ? Center(
                    child: Padding(
                      padding: const EdgeInsets.all(24),
                      child: Text(
                        i18n('cookie_capture_unsupported'),
